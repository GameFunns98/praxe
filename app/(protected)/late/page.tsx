...
export default async function LatePage() {
  ...
  return <div className="space-y-4">
    ...
    {records.length === 0 ? <div className="rounded-xl border border-dashed bg-white p-8 text-center text-slate-500">Bez pozdních záznamů.</div> : records.map((r) => <div key={r.id} className="rounded-xl border bg-white p-4 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-2"><p className="font-medium">{r.trainee.fullName}</p><PracticeStatusBadge status={r.status} /></div><p className="text-sm text-slate-600">Dohlížející: {r.supervisor.fullName}</p><p className="text-sm text-slate-600">Délka: {formatMinutes(r.durationMinutes)}</p><Link href={`/practice/${r.id}`} className="mt-3 inline-flex rounded-md border px-3 py-1.5 text-sm">Otevřít detail</Link></div>)}
  </div>;
}
