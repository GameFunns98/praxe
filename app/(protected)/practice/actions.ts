...
export async function approvePracticeRecord(formData: FormData) {
  ...
  const record = await prisma.practiceRecord.findUnique({ where: { id: parsed.id } });
  ...
  await logAudit(session.user.id, 'PracticeRecord', record.id, 'APPROVE', record, { status: 'APPROVED' });
  revalidatePath('/practice');
...
export async function rejectPracticeRecord(formData: FormData) {
  ...
  await logAudit(session.user.id, 'PracticeRecord', record.id, 'REJECT', record, { status: 'REJECTED' });
  revalidatePath('/practice');
}
